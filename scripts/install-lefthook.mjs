...
async function main() {
  ...
  // Lefthook is a devDependency; a production install (pnpm install
  // --production) legitimately omits it. Without the package there are no
  // hooks to install, so exit quietly instead of failing the install.
  let lefthookPackage
  try {
