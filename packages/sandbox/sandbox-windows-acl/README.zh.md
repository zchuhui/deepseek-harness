...
- **宽目录、FAT 卷与首次授权延迟警告是操作员日志。** 该工作区第一次常驻 ACE 会通过 `ctx.logger` 记录这些警告；它们不改变 `enforcement: 'partial'`。FAT 卷作为授权**根**仍会大声失败（无 ACL 支持）。授权根**之外**的 FAT 类目标没有安全描述符，因此受限令牌的写检查通过（Everyone 在两种列表中都在）——此类目标在**两种**受限模式下都可写。FAT 被视为遗留残留——不受支持、不围绕它设计。
