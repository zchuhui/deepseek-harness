...
- **Wide-directory, FAT-volume, and first-grant-latency warnings are operator logs.** The first standing workspace ACE logs those warnings through `ctx.logger`; they do not change `enforcement: 'partial'`. A FAT volume as a grant ROOT still fails the grant loudly (no ACL support). A FAT-class target OUTSIDE the granted roots has no security descriptors, so the restricted token's write check passes (Everyone sits in both lists) and such targets are writable under BOTH confined modes. FAT is treated as a legacy residue — unsupported and not engineered around.
