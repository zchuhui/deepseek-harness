...
| `terminal-bash`（`@deepseek-ai/dsh-terminal-bash`） | `ctx.subprocess.spawnTerminal` 之上的 bash 后端：就绪检测、有界终端状态、沙箱策略与会话操作 | 注册到 `ctx.terminals` |
| `terminal-pwsh`（`@deepseek-ai/dsh-terminal-pwsh`） | 同一原语之上的 PowerShell 后端；Windows 组合选择加入，受限 PTY 会被拒绝 | 注册到 `ctx.terminals` |
