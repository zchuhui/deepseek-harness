...
| `terminal-bash` (`@deepseek-ai/dsh-terminal-bash`) | Bash backend over `ctx.subprocess.spawnTerminal`: readiness detection, bounded terminal state, sandbox policy, and session operations | registers on `ctx.terminals` |
| `terminal-pwsh` (`@deepseek-ai/dsh-terminal-pwsh`) | PowerShell backend over the same primitive; Windows compositions opt in, confined PTY is refused | registers on `ctx.terminals` |
