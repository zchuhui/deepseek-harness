...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    ...
    host: {
      ...
      reportWindow: request => ok(request, { reported: true as const }),
    },
    desktop: {
      getSettings: request => ok(request, { closeToTray: false, launchAtLogin: false }),
      setSettings: request => ok(request, {
        closeToTray: request.payload.closeToTray ?? false,
        launchAtLogin: request.payload.launchAtLogin ?? false,
      }),
    },
    workspace: {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'desktop.getSettings': return this.api.desktop.getSettings(request)
      case 'desktop.setSettings': return this.api.desktop.setSettings(request)
