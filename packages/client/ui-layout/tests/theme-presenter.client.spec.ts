...
import { DARK_ATTRIBUTE, THEME_ATTRIBUTE, ThemePresenter } from '@deepseek-ai/dsh-client-ui-layout/src/client/theme-presenter.ts'

const LIGHT_THEME_COLOR = 'rgb(255, 255, 255)'
const DARK_THEME_COLOR = 'rgb(21, 21, 23)'

function snapshot(
  colorScheme: 'light' | 'dark', tokens: Record<string, string> = {}, id = `${colorScheme}-test`,
): ThemeSnapshot {
  // The presenter must key off colorScheme, not the id — keep them distinct.
  const active = { id, colorScheme, tokens }
...
describe('ThemePresenter', () => {
  ...
  it('retains the concrete glass id while selecting the dark base palette', () => {
    const presenter = new ThemePresenter()
    presenter.apply(snapshot('dark', {}, 'glass-obsidian'))
    expect(document.body.hasAttribute(DARK_ATTRIBUTE)).toBe(true)
