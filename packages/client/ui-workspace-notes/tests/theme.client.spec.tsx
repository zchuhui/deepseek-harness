...
/**
 * Four-theme coverage for the notes tab, in two halves. The CSS half reads
 * the component sheets and the ui-theme token sheets from disk: every
 * `var(--dsw-*)` the components reference is defined by both the light base
...
const componentSheets = [
  '../src/client/NotesPane.module.css',
  '../src/client/MessageNoteAction.module.css',
].map(readSibling)
...
describe('notes tab theme coverage (CSS contract)', () => {
  ...
  it('keeps no literal color in the component sheets', () => {
    for (const sheet of componentSheets) {
...
describe('notes tab theme coverage (DOM invariance)', () => {
  it('renders identical markup under every theme and keeps the draft across switches', async () => {
    ...
    const carried = { ok: true as const, value: { ok: true as const, value: { notes: [note], familyRevision: 1 } } }
    const remote = {
      list: () => Promise.resolve(carried),
      create: () => Promise.resolve(carried),
      update: () => Promise.resolve(carried),
    ...
    const props = {
      workspaceId: WS,
      managerFor: () => manager,
      actions: new WorkspaceNotesActions(remote as never),
      t,
    } as unknown as NotesPaneViewProps

    const { container } = render(<NotesPane {...props} />)
