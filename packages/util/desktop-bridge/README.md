...
Zero-dependency typed client for the desktop shell's native bridge: a token-guarded loopback HTTP API through which dsh host providers reach toast, directory-picker, keychain, window, settings, and updater primitives. The wire contract is owned by `desktop-app/README.md`; this package only types the client.
