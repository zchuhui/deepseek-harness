...
import { WorkspaceNotesManager } from '../src/manager.ts'
import type { WorkspaceNotesRemoteFace } from '../src/manager.ts'
import type { WorkspaceNote, WorkspaceNotesListResult } from '../src/types.ts'
...
const note = (id: string, updatedAt: string): WorkspaceNote => ({
  noteId: id as WorkspaceNote['noteId'],
...
type RemoteOutcome =
  | { ok: true; value: WorkspaceNotesListResult }
  | { ok: false; error: { code: string; message: string; details: object } }
  | Error

/** Scripted remote double recording `list` calls; the last response repeats. */
function scriptedRemote(responses: readonly RemoteOutcome[]): WorkspaceNotesRemoteFace & { listCalls: number } {
  let calls = 0
  const remote: WorkspaceNotesRemoteFace & { listCalls: number } = {
    listCalls: 0,
    list: async (request) => {
...
const carried = (value: WorkspaceNotesListResult) => ({ ok: true as const, value })
const carrierFailure = { ok: false as const, error: { code: 'unavailable', message: 'gateway is down', details: {} } }

describe('workspace notes manager', () => {
  it('publishes the ready baseline with the host-ordered list', async () => {
    const older = note('a', '2026-08-15T00:00:00.000Z')
    const newer = note('b', '2026-08-15T01:00:00.000Z')
    const remote = scriptedRemote([carried({ ok: true, value: { notes: [newer, older], familyRevision: 1 } })])
    const manager = new WorkspaceNotesManager(remote, WORKSPACE)
    ...
    expect(view.notes).toEqual([newer, older])
  })

  it('collapses concurrent refreshes onto one in-flight read', async () => {
    const remote = scriptedRemote([carried({ ok: true, value: { notes: [], familyRevision: 1 } })])
    const manager = new WorkspaceNotesManager(remote, WORKSPACE)
  ...
  it('replays a frame that lands while a baseline is in flight', async () => {
    let reads = 0
    const remote: WorkspaceNotesRemoteFace = {
      list: async () => {
        reads += 1
        return carried({ ok: true, value: { notes: [], familyRevision: 1 } })
      },
    }
    const manager = new WorkspaceNotesManager(remote, WORKSPACE)
  ...
  it('surfaces a carrier failure as an error view without losing the list', async () => {
    const kept = note('a', '2026-08-15T00:00:00.000Z')
    const remote = scriptedRemote([
      carried({ ok: true, value: { notes: [kept], familyRevision: 1 } }),
      carrierFailure,
    ])
    const manager = new WorkspaceNotesManager(remote, WORKSPACE)
    await manager.refresh()
    ...
    expect(view.notes).toEqual([kept])
  })
